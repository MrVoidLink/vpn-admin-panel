...
const isHostLike = (s) =>
  /^(\d{1,3}\.){3}\d{1,3}$|^([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/.test(S(s).trim());
                  ...
                  <div className="flex items-center justify-between">
                    <p className="font-semibold capitalize">
                      ...
                      {v.protocol === "openvpn" ? (
                        ...
                        <Badge color="purple" text={`${v.endpointHost || "host"}:${v.endpointPort ?? "-"}`} />
                      )}
                    </p>
...
function validateVariant(v) {
  ...
  if (v.protocol === "openvpn") {
    if (!["udp", "tcp"].includes(S(v.ovpnProto).toLowerCase())) {
  ...
  if (v.protocol === "wireguard") {
    if (v.endpointHost && !isHostLike(v.endpointHost)) {
      errs.push("wireguard.endpointHost is invalid");
    }
